/*
# Fix reposition function v3: use generate_series for proper row numbering

The previous version had a bug where ROW_NUMBER() OVER () on ST_Dump results
didn't produce unique row numbers. This version uses generate_series to create
a proper index for joining water cats to new land points.
*/
...
BEGIN
  ...
  -- Count how many water cats we'll process
  ...
  -- Use generate_series to create a proper index for both sides
  WITH water_cats AS (
